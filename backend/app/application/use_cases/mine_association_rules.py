...
from app.application.ports.analysis_services import DatasetReader, AssociationRulesService
...
class MineAssociationRulesUseCase:
    def __init__(self, datasets: DatasetRepository, results: AnalysisResultRepository, reader: DatasetReader, service: AssociationRulesService):
        self.datasets = datasets
        self.results = results
        self.reader = reader
        self.service = service

    def execute(self, dataset_id: str, columns: list[str], min_support: float, min_confidence: float) -> AnalysisResult:
        if not columns:
            raise InvalidAnalysisInputError("Informe ao menos uma coluna categórica para regras de associação")
        ...
        result = AnalysisResult(id=str(uuid4()), dataset_id=dataset_id, analysis_type="association_rules", payload=payload)
