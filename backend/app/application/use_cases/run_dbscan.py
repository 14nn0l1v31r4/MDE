...
class RunDBSCANUseCase:
    ...
    def execute(self, dataset_id: str, columns: list[str] | None, eps: float, min_samples: int) -> AnalysisResult:
    def execute(
        self,
        dataset_id: str,
        ...
        payload = self.service.run_dbscan(df, columns, eps, min_samples)
        result = AnalysisResult(id=str(uuid4()), dataset_id=dataset_id, analysis_type="dbscan", payload=payload)
        result = AnalysisResult(
            id=str(uuid4()),
            dataset_id=dataset_id,
            analysis_type="dbscan",
            payload=payload,
            user_id=user_id,
        )
